...
- **Edit, rename, delete.** Press **⌘E** on a room in ⌥Space (or right-click it, or use the menu for the room you're in) to choose its windows again and rename it; when you're done, ⌥Space comes back on that room so Tab and ⇧Tab can lay it out with its new windows. Delete a room with the ⓧ on its row, a right-click, or **⌘⌫** in ⌥Space, and **⌘Z** brings it back. Deleting a room never touches its windows.
